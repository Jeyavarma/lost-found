          ...
          {eventsData.map((event) => (
            <Card key={event.id} className="mcc-card hover:shadow-xl transition-all duration-300 border-2 border-brand-primary/20 group cursor-pointer relative w-80 flex-shrink-0">
              ...
              <CardContent className="p-6">
                ...
                <Button 
                  className="w-full mcc-accent hover:bg-[#1e4765]" 
      ...
      <Dialog open={!!selectedItem} onOpenChange={() => setSelectedItem(null)}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-6">
            ...
            <Button className="w-full mcc-accent hover:bg-[#1e4765]" onClick={() => window.open(`mailto:lostfound@mcc.edu.in?subject=Regarding ${selectedItem?.name}&body=Hi, I am contacting you regarding the ${selectedItem?.status} item: ${selectedItem?.name}. Location: ${selectedItem?.location}. Date: ${selectedItem?.date}.`)}>
